 ...
 * Copyright 2023 Aleks Rutins <aleks@rutins.com>
 *
...
struct _SpectrumScheme {
  GObject parent_instance;

  GtkSourceStyleScheme *style;
};

G_DEFINE_TYPE (SpectrumScheme, spectrum_scheme, G_TYPE_OBJECT)

static gboolean _editor_source_style_scheme_is_dark (GtkSourceStyleScheme *);

static void
spectrum_scheme_class_init (SpectrumSchemeClass *klass)
{}

static void
spectrum_scheme_init (SpectrumScheme *self)
{}

SpectrumScheme *
spectrum_scheme_new (void)
{
  return g_object_new(SPECTRUM_TYPE_SCHEME, NULL);
}

SpectrumScheme *
spectrum_scheme_new_for_gtksource (GtkSourceStyleScheme *style)
{
  SpectrumScheme *self = spectrum_scheme_new();
  spectrum_scheme_load (self, style);
  return self;
}

void
spectrum_scheme_load (SpectrumScheme       *self,
                      GtkSourceStyleScheme *style)
{
  self->style = style;
}
              ...
              const GdkRGBA *color)
{
  ...
  g_string_append_printf (str, "@define_color %s %s;\n", name, color_str);
}
...
GString *
spectrum_scheme_generate_css (SpectrumScheme *self)
{
  ...
  gboolean has_fg;
  gboolean has_bg;
  GtkSourceStyleScheme *style_scheme = self->style;
...
spectrum_scheme_get_variant (SpectrumScheme *self,
                             const char     *variant)
{
  ...
  const char *mapping;
  GtkSourceStyleScheme *scheme = self->style;
...
void
spectrum_scheme_apply (SpectrumScheme *self)
{
  g_autoptr(GString) css = spectrum_scheme_generate_css (self);
